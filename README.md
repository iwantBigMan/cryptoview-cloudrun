...
- `riskTags`는 앱이 계산한 리스크 신호이며 투자 추천이 아니라고 명시합니다.
- LLM은 제공된 데이터와 `riskTags`에 근거한 설명만 수행합니다.
...
curl.exe -X POST "http://localhost:8080/api/ai/portfolio-insight" `
  -H "Authorization: Bearer YOUR_FIREBASE_ID_TOKEN" `
  -H "Content-Type: application/json" `
  -d "{\"baseCurrency\":\"KRW\",\"totalValuationKrw\":6862.62,\"totalPnlKrw\":-1595.23,\"totalPnlRate\":-18.86,\"holdings\":[{\"exchange\":\"Gate.io\",\"symbol\":\"SOL\",\"quantity\":0.01798218,\"valuationKrw\":2400,\"portfolioRatio\":34.9,\"averagePrice\":94.48,\"currentPrice\":96.12,\"pnlKrw\":40,\"pnlRate\":1.7,\"riskTags\":[\"concentration_medium\"]}]}"
```
